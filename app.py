...
def import_and_predict(test):
    models = load_model('models/simple_classifier/Epoch-50-Val_acc1.00.hdf5')
    lbs = open('models/simple_classifier/label_transform.pkl', 'rb')
    k = predi(test, models, lbs)
    if k == 'Banana':
        model = load_model('models/banana_classifier/Epoch-48-Val_acc0.95.hdf5')
        lb = open('models/banana_classifier/label_transform.pkl', 'rb')
        ...
        model = load_model('models/paddy_classifier/Epoch-05-Val_acc0.82.hdf5')
        lb = open('models/paddy_classifier/label_transform.pkl', 'rb')
        pred = predic(test, lb, model)
        del model
        del lb
        gc.collect()
        ...
        model = load_model('models/tomato_classifier/Epoch-29-Val_acc0.99.hdf5')
        lb = open('models/tomato_classifier/label_transform.pkl', 'rb')
        pred = predic(test, lb, model)
        del model
        del lb
        gc.collect()   
